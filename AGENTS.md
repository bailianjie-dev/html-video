...
├── AGENTS.md                  本文件，内部 working notes
├── LICENSE                    Apache-2.0
├── .gitignore                 通用
...
**待 Joey 定（git 写操作，未执行）**：① 本轮改动开 PR（按惯例对外可见/新模板走 PR review）还是直推；② 上一会话(6/6)遗留的 AGENTS.md 39 行未提交记录怎么处理（AGENTS.md 是追踪文件但属内部 notes，不宜混进代码 PR）。临时 driver 在 `~/Desktop/Codex/scratch/hv-remotion-native/`（不进库）。
...
> 上游 clone 仍在 `~/Desktop/Codex/scratch/upstream-check/`（huashu-design + frontend-slides，可能被定期清理）。

## 跑起来

```bash
cd ~/Desktop/Codex/projects/html-video
pnpm install
pnpm -r build
